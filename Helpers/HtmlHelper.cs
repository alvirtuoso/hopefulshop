...
namespace shop.Helpers
{
    public static class HtmlHelper
    {
        ...
        /// <summary>
        /// Gets the dns domains associated with account api-key
        /// </summary>
        public static void GetDns(string apiKey){
            byte[] data = Encoding.ASCII.GetBytes("DCID=24&VPSPLANID=201&OSID=152");
            try
            {
                WebRequest request = (HttpWebRequest)WebRequest.Create("https://api.vultr.com/v1/dns/list");
        ...
        public static void CreateRecord(string key, shop.Models.Dns dns)
        {
            ...
            //string dnsData = "domain=hopeful.shop&name=ttest&type=A&data=192.0.2.1";
        ...
        public static String EndPointUrl(String countryCode){
        ...
        /// <param name="currency">Currency sign/symbol</param>
        public static float FindDisplayedPrice(XmlNode offersNode, out string currency)
        {
            float n = 0;
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                if (nodes[i].Name == "Offer")
                {
                    ...
                    if (ol != null && ol.HasChildNodes){
                        ...
                        if(priceNode != null){
                            var fp = HtmlHelper.FindNode("FormattedPrice", priceNode);
                            currency = fp.InnerText[0].ToString();
                            // Remove the currency sign first then parse to float
                            float.TryParse(fp.InnerText.Remove(0, 1), out n);
                        }
                    }
